...
vector<int> BronKerbosch(vector<int> &P, vector<int> &R, vector<int> &X, Graph G){
	...
	for(int i = 0; i < P.size(); i){
		...
		vector<int> voisinsV = graph->GetVoisins(P[i]);
		...
		BronKerbosch(interPVoisinsV, unionRetV, interXVoisinsV,G);
		X.push_back(P[i]);
		P.erase(P.begin());
...
vector<int> BronKerboschDegeneracy(Graph G){
    
	int positionSommetOrdreDege[G->GetNbSommets()];
	vector<int> vectOrdreDegenerescence;
	Graph* copieGraphe = Graph->CreerCopie();
	ordreDegenerescence(copie_graphe,vectOrdreDegenerescence,positionSommetOrdreDege);
	...
	for(int v : vectOrdreDegenerescence){
		...
		vector<int> voisinsV = graph->GetVoisins(v);

		for(int i = 0; i < voisinsV.size(); i++){
		...
		BronKerboschPivot(P, ensembleV, X,G);
...
vector<int> BronKerboschPivot(vector<int> &P, vector<int> &R, vector<int> &X, Graph G){
	...
	voisinsPivot = graph->GetVoisins(pivot);
	...
	for(int v : PPriveVoisinsPivot){
		...
		BronKerboschPivot(interPVoisinsV, unionRetV, interXVoisinsV,G);
...
void ordreDegenerescence(Graph graphe, vector<int> &tabOrdreDege, int posSommetOrdreDege[]){
	...
	int maxDeg = 0;
	int degSommet;
	for(int v : graphe->GetListeSommets()){
		degSommet = graphe->GetDegres(v);
		if(degSommet > maxDeg){
			maxDeg = degSommet;
		}
	}
	...
	for(int v : graphe->GetListeSommets()){
		degSommet = graphe->GetDegres(v);
		tabDegres[degSommet].push_back(v);
	}
	...
	while(graphe->GetNbSommets() > 0){
		...
		int degreVoisin;
		for(int v : voisinsSommetASup){
			degreVoisin = graphe->GetDegres(v);		
			tabDegres[degreVoisin].erase(std::remove(tabDegres[degreVoisin].begin(), tabDegres[degreVoisin].end(), v), tabDegres[degreVoisin].end());
			tabDegres[degreVoisin-1].push_back(v);
		}
